...
from quizzes.models import Cafedra, Course, User, Quiz, Questions, Answers, QuizSolveRecord
...
from quizzes.repositories import QuizRepository, CourseRepository, QuestionRepository, AnswerRepository, QuizSolveRecordRepository
...
def view_quiz_for_check(request, pk):
	quiz = QuizRepository(Quiz)
	owner_id = quiz.get_owner_id(pk)
	qsr = QuizSolveRecordRepository(QuizSolveRecord)
	quizzes = qsr.get_quizzes_for_check(pk)
	course_id  = quiz.get_course_id(pk)
	if request.user.id == owner_id:
...
def get_answers_for_check(request, pk, spk):
	...
	return render(request, 'teachers/check_quiz/check_quiz.html', {'answers': answers})
